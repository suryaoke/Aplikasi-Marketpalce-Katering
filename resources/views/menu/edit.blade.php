...
@section('admin')
    ...
    <form method="post" action="{{ route('menu.update') }}" enctype="multipart/form-data" id="myForm">
        @csrf
        ...
        <div class="mt-3">
            ...
            <input type="text" class="intro-x login__input form-control py-3 px-4 block mt-1"
                placeholder="Masukkan Deskripsi" name="deskripsi" value="{{ $menu->deskripsi }}">
        </div>
        <div class="mt-3">
            <label>Harga</label>
            <input type="text" class="intro-x login__input form-control py-3 px-4 block mt-1"
                placeholder="Masukkan Harga" name="harga" pattern="[0-9]+" title="Please enter only numbers"
                value="{{ $menu->harga }}">
        </div>
        ...
        <div class="mt-3 flex ">
            <img width="130px auto" id="showImage"
                src="{{ !empty($menu->foto) ? url('uploads/' . $menu->foto) : url('uploads/no_image.jpg') }}"
                alt="Card image cap">
